...
namespace ConsoleApp7
{
    public static class ReaderManager
    {
        ...
        public static List<Reader> SearchByLastName(string lastName) =>
            ...
            _readers.FindAll(r => r.LastName.Contains(lastName, StringComparison.OrdinalIgnoreCase));
        ...
        public static void ShowReaders(List<Reader> list)
        {
            ...
            string header = $"  {"ID",-5} {"Фамилия",-18} {"Имя",-18} {"Адрес",-25} Возраст";
            int padding = (windowWidth - header.Length) / 2;
            if (padding < 0) padding = 0;
            Console.WriteLine(new string(' ', padding) + header);
            Console.WriteLine(new string(' ', padding) + new string('-', Math.Min(80, windowWidth - padding)));
            Console.ResetColor();

            foreach (var r in list)
            {
                string line = $"  {r.Id,-5} {FormatCell(r.LastName, 18)} {FormatCell(r.FirstName, 18)} {FormatCell(r.Address, 25)} {r.Age}";
